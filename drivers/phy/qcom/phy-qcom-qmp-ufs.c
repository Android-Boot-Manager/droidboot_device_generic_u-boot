...
static int qmp_ufs_probe_dt_children(struct udevice *dev)
{
	...
	struct qmp_ufs_priv *qmp = dev_get_priv(dev);
	struct qmp_ufs_cfg *cfg = (void*)dev_get_driver_data(dev);
	...
	if (!found_children) {
		qmp->tx = qmp->serdes + cfg->offsets->tx;
		qmp->rx = qmp->serdes + cfg->offsets->rx;
		qmp->pcs = qmp->serdes + cfg->offsets->pcs;
		if (cfg->lanes >= 2) {
			qmp->tx2 = qmp->serdes + cfg->offsets->tx2;
			qmp->rx2 = qmp->serdes + cfg->offsets->rx2;
		}
	}
